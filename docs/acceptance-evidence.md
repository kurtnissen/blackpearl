...
| Paired acquisition UI | Pass in Vitest and live Brave rendering | Seventeen frontend tests cover typed API validation, Prowlarr connection, cached movie acquisition, secret-free browser storage, existing manual selection, and provider errors. The embedded production build restored the four-item live manifest and rendered the acquisition panel with the internal Prowlarr URL after stack rebuild. |
