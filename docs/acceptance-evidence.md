...
| Durable Watchlist acquisition | Pass in automated tests; partial live provider acceptance | One newly observed movie enters the existing restart-safe acquisition queue without invoking the cached-only coordinator. The linked job ID is committed before releasing the Watchlist lease; active, succeeded, no-release/stalled, unauthorized, and manual-review outcomes reconcile to privacy-safe Watchlist state. Cancellation-safe commits, startup baseline protection, and serialization pass under `-race`. During live acceptance, the first opt-in exposed that historical pending rows were also eligible: automatic intake was stopped, the one exact accidental provider object was deleted, its job was quarantined, and immutable post-baseline eligibility was added. After rebuild, only the newly added Creative Commons *Big Buck Bunny* item entered the queue; its selected Archive torrent reached TorBox's terminal stalled state, was not published, and the exact test object was deleted. No acquisition jobs remain active. A post-fix Watchlist-triggered successful publication remains pending. |
...
- [ ] A post-baseline legal Watchlist item completes provider preparation, publication, Plex Direct Play, and bidirectional seeking on the final safety-fixed build.
