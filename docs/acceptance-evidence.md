...
| Durable Watchlist acquisition | Pass (mocked full process) | One observed movie enters the existing restart-safe acquisition queue without invoking the cached-only coordinator. The linked job ID is committed before releasing the Watchlist lease; active, succeeded, no-release/stalled, unauthorized, and manual-review outcomes reconcile to privacy-safe Watchlist state. Cancellation-safe commits and serialization pass under `-race`. Shows remain observation-only. Live Watchlist-triggered mutation remains pending. |
