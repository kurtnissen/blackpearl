...
| Go coverage | Pass | 80.2% of project statements from `go test -coverprofile=coverage.out ./...`; generated Go code inside `web/node_modules` is excluded from the project floor |
...
| Durable acquisition state machine | Pass in race tests and live process replay | SQLite jobs preserve only validated intent, release fingerprints, provider IDs, leases, transitions, and public error codes. Tests cover restart recovery, one-winner claims, stale leases, redaction, reconcile-before-create, bounded verified materialization, terminal stalls, completed-but-not-present polling, missing-object manual review, and atomic publication. A live durable replay of *Tears of Steel (2012)* reconciled the prepared TorBox object and reached `succeeded` through the background worker. |
